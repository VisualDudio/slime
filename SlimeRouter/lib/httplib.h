...
inline socket_t create_socket_unix(const char *path) {
  ...
  if (bind(sock, (sockaddr *) &addr, sizeof(addr.sun_family) + strlen(addr.sun_path)) < 0) {
    std::cout << "Cannot bind socket\n";
    return sock;
  }
  if (listen(sock, 128) < 0) {
    std::cout << "Cannot listen socket\n";
    return sock;
  }
  return sock;
}
...
Server::create_server_socket(const char *host, int port, int socket_flags,
    ...
    (void)socket_flags;
    (void)socket_options;
    return detail::create_socket_unix(host);
